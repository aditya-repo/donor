...
export const cityField = Joi.string()
    .trim()
    .messages({
        "string.base": "City must be string",
    });

export const bloodGroupField = Joi.string()
    .trim()
...
export const bloodTypeField = Joi.string()
    .valid(
        ...
        "Plasma",
        "R.B.C",
        "S.D.P (Single Donor Platlets",
        "Others"
    )
    .required()
    .messages({
        "any.only":
            "Blood type must be one of PCV, Whole Blood, Platelets, Plasma, R.B.C, S.D.P (Single Donor Platlets, Others",
        "any.required": "Blood type is required",
    });

export const unitcountField = Joi.number()
    .integer()
    .positive()
    .required()
    .messages({
...
export const statusField = Joi.string()
    .valid("Rush", "Urgent", "Planned", "Other")
    .required()
    .messages({
...
export const requestnoteField = Joi.string()
    .trim()
    .max(500)
    .messages({
...
export const stateField = Joi.string()
    .trim()
    .min(2)
    .max(50)
    .required()
...
export const districtField = Joi.string()
    .trim()
    .min(2)
    .max(50)
    .required()
...
export const landmarkField = Joi.string()
    .trim()
    .min(2)
    .max(100);

export const hospitalnameField = Joi.string()
    .trim()
    .min(2)
    .max(100)
    .required()
